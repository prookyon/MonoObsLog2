...
void CamerasTab::populateTable()
{
    ...
    for (const auto &[id, name, sensor, pixelSize, width, height] : cameras)
    {
        ...
        // Sensor column
        auto *sensorItem = new QTableWidgetItem(sensor);
        ui->camerasTable->setItem(row, 1, sensorItem);

        // Pixel Size column
        auto *pixelSizeItem = new QTableWidgetItem(QString::number(pixelSize, 'f', 2));
        ...
        // Height column
        auto *heightItem = new QTableWidgetItem(QString::number(height));
...
bool CamerasTab::showCameraDialog(const QString &title, QString &name, QString &sensor, double &pixelSize, int &width, int &height)
{
    ...
    connect(buttonBox, &QDialogButtonBox::accepted, &dialog, [nameEdit, sensorEdit, &dialog]()
            {
...
void CamerasTab::onAddCameraButtonClicked()
{
    ...
    const QString sensor = ui->sensorLineEdit->text().trimmed();
    const double pixelSize = ui->pixelSizeSpinBox->value();
...
void CamerasTab::onEditCameraButtonClicked()
{
    ...
    const int currentRow = ui->camerasTable->currentRow();
    if (currentRow < 0)
    {
    ...
    const QTableWidgetItem *nameItem = ui->camerasTable->item(currentRow, 0);
    const int cameraId = nameItem->data(Qt::UserRole).toInt();
    const QString currentName = nameItem->text();
    const QString currentSensor = ui->camerasTable->item(currentRow, 1)->text();
    const double currentPixelSize = ui->camerasTable->item(currentRow, 2)->text().toDouble();
    const int currentWidth = ui->camerasTable->item(currentRow, 3)->text().toInt();
    const int currentHeight = ui->camerasTable->item(currentRow, 4)->text().toInt();
...
void CamerasTab::onDeleteCameraButtonClicked()
{
    ...
    const QTableWidgetItem *nameItem = ui->camerasTable->item(currentRow, 0);
    const int cameraId = nameItem->data(Qt::UserRole).toInt();
    const QString cameraName = nameItem->text();
